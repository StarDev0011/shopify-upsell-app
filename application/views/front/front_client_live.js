if (window.location.href.indexOf('https://gretrix-testing.myshopify.com/') >= 0) {
    base_url = 'https://dev.smartcartupsellbundle.com/';
} else {
    base_url = 'https://smartcartupsellbundle.com/';
}
...
//    return false;
    ...
    if ((loadedPage)) { //Current page is product details and standard cross sell added then show upsellpopup on click of add to cart
        var target = $(e.target);
//        console.log(target);
//        console.log(target.parent().is("button[name='add']"));
        ...
        if (localStorage.getItem('isCrossSellAdded') != 1 && (typeof productSlug=='undefined' || productSlug=='')) {
          //  showPopup(e);
        	console.log("checking length " + $('.ajaxcart__row').length)
        	if($('.ajaxcart__row').length > 1)
        		{
        			console.log("popup");
        			// showCartPopup();
        		}
...
setInterval(function(){
    ...
    $.ajax({
		...
		if(cart_items.length > 1)
			...
			$.ajax('https://laserglowskincare.com/discount/20OffIPL');
			}
       else
       {
         if(cart_items.length != "undefined"  && cart_items.length <= 1)
         {
        	 $.post('https://laserglowskincare.com/checkout?discount=%20');
...
setInterval(function() {
	...
	console.log('SLUGGGG ' + productSlug)
},3000);

function showPopup(e) {
    console.log("showPopup() executed");
    var variantId = $("[name='id']").val();
    var result = 1;
    var pId = $('#pr_id').val();
...
//	if((window.location.href).indexOf("laserglowskincare.com") == -1)
		var urlll = 'front/getCartPagePopup';
//	else
//		var urlll = 'front/getCartPagePopup2';
 ...
 * @return {Number}
 */
function getPOPUpProductsChecker(shopId, cart_items, cart_price, cart_token, isChangeContent) {
    var cnt = 0;
    data = {'shopId': shopId, 'cart_price': cart_price, 'last_added_bundle': localStorage.getItem('latestBundle'), 'goal_amount': localStorage.getItem('goalAmount'), 'latest_used_bundle': localStorage.getItem('lastUsedBundle'), 'last_used_variant': localStorage.getItem('lastUsedVariant'), 'cart_items': cart_items, 'product_slug': productSlug, 'slugType': typeof productSlug};
    var goalAmount = parseFloat(localStorage.getItem('goalAmount'));
    $.ajax({
        ...
        error: function (textStatus, errorThrown) {
        	getPOPUpProductsChecker(shopId, cart_items, cart_price, cart_token, isChangeContent)
        }
    });
    return cnt;
}
...
function extractProductSlug(url) {
	var productSlugOld = productSlug;
...
//    getCurrentCartItems(1);
//    productSlug = slug;
//    if(getPOPUpProductsChecker(shopId, cart_items, cart_price, cart_token, 1) != 1)
//    	return productSlugOld;
