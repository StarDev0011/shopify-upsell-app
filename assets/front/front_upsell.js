	...
	console.log("ZACH TEST PUSHED CORRECTLY")
	 ...
	 */
	 console.log("Test:", $(checkoutButtonSelector));
	...
	function addItem(form_id, isReachedGoal) {
		...
		var discountTitle = $('.discount_code_' + form_id).val();
		var qtyAry = [];
		qtyAry.push({
			formId: form_id,
	...
	function addUpsellItem(variants, isDivQuantity, quantity, isLoop, isReachedGoal) {
		...
		if (variants.length) {
			...
			console.log('#upsell-item-success_' + i.formId);
			$.ajax({
				url: "/cart/add.js",
				type: "POST",
				dataType: "json",
				...
				success: function (data) {
					...
					if (noOfBundle > 1 || (discount_type != 0 && discount_type != 3)) {
						...
						$('button.pro_' + overallProductId).text("Added To Cart");
						$('button.pro_' + overallProductId).css('background', '#34bf55');
						$('.check-out.popup_checkout').click();
						setTimeout(function () {
							$('#item_added_success').fadeOut();
							$('button.pro_' + overallProductId).css('background', '#287dee');
